...
export const getDepartments = async () => {
  try {
    ...
    // Departamentos
    const departmentsResponse = await axios.get(`http://api.geonames.org/childrenJSON?geonameId=3686110&username=${GEO_NAMES_USERNAME}`);
    console.log("Response from GeoNames for departments:", departmentsResponse.data); // Agregar esta línea
...
export const getCities = async (id) => {
  try {
    ...
    // Ciudades
    const citiesResponse = await axios.get(`http://api.geonames.org/childrenJSON?geonameId=${id}&username=${GEO_NAMES_USERNAME}`);
